...
st.markdown(
    ...
    " across custom screeners including VCP concepts, 52-week highs, ADTV"
    " liquidity, and Relative Strength filters."
)
...
selected_directory_sector = st.selectbox(
    ...
    list(sector_stocks_db.keys()),
    key="directory_sector_select",
)
...
# ==========================================
# PART 4: PARTITION 3 - VCP CONCEPT SCREENER
# ==========================================
st.markdown("---")
st.header("⚡ Partition 3: VCP Concept Screener (Volatility Contraction Pattern)")
st.markdown(
    "Scanning Nifty 500 stocks for Minervini VCP setups: uptrending 50/200 SMAs,"
    " price near all-time highs, range contraction, and significant volume"
    " dry-up."
)
...
def run_vcp_screener():
  tickers = get_nifty500_tickers()
  vcp_results = []
  progress_bar_vcp = st.progress(0)
  total_stocks = len(tickers)

  for i, ticker in enumerate(tickers):
    progress_bar_vcp.progress((i + 1) / total_stocks)
    try:
      df = yf.download(ticker, period="1y", interval="1d", progress=False)
      if not df.empty and len(df) >= 200:
        if isinstance(df.columns, pd.MultiIndex):
        ...
        sma_200_curr = sma_200.iloc[-1]
        sma_200_20ago = (
            sma_200.iloc[-20] if len(sma_200) >= 20 else sma_200.iloc[0]
        )

        low_252 = low.tail(252).min()
        high_252 = high.tail(252).max()
        vol_sma_20 = vol.rolling(20).mean().iloc[-1]

        # VCP Logical Rules as requested
        c1 = curr_close > sma_50
        c2 = sma_50 > sma_200_curr
        c3 = sma_200_curr > sma_200_20ago
        c4 = curr_close >= (low_252 * 1.3)
        c5 = curr_close >= (high_252 * 0.75)
        c6 = (high.iloc[-1] - low.iloc[-1]) < (high.iloc[-5] - low.iloc[-5])
        c7 = curr_vol < (vol_sma_20 * 0.6)
        c8 = curr_vol < vol.iloc[-2]
        c9 = curr_close > 50
        c10 = vol_sma_20 > 100000
        ...
        if (
          ...
          pct_below_ath = (high_252 - curr_close) / high_252
          score = (1 - pct_below_ath) * 100 + (curr_vol / vol_sma_20) * 10

          vcp_results.append({
              "ticker": ticker.split(".")[0],
              "price": curr_close,
              "high_252": high_252,
              "distance_ath": f"{pct_below_ath * 100:.2f}%",
              "score": score,
          })
    except Exception:
      pass

  progress_bar_vcp.empty()
  vcp_results = sorted(vcp_results, key=lambda x: x["score"], reverse=True)
  return vcp_results[:10]


vcp_stocks = run_vcp_screener()

if not vcp_stocks:
  st.info(
      "No stocks currently match all strict VCP volatility contraction criteria."
  )
else:
  st.success(
      f"Found {len(vcp_stocks)} top-tier VCP Contraction & Breakout setups!"
  )
  cols_vcp = st.columns(3)
  for idx, stock in enumerate(vcp_stocks):
    with cols_vcp[idx % 3]:
      st.markdown(
          f"### 🧬 `{stock['ticker']}`\n"
          f"**Price:** ₹{stock['price']:,.2f}  \n"
          f"**52W/ATH High:** ₹{stock['high_252']:,.2f}  \n"
          f"**Distance to High:** {stock['distance_ath']}"
      )
      st.markdown("---")
