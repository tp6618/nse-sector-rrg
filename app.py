...
if not consensus_stocks:
  ...
  for idx, stock in enumerate(consensus_stocks[:9]):
    with cols[idx % 3]:
      ...
      st.markdown("---")    "FIN SERVICE": "^CNXFIN",
    "FMCG": "^CNXFMCG",
    "IT": "^CNXIT",
    "MEDIA": "^CNXMEDIA",
    "METAL": "^CNXMETAL",
...
def run_52w_high_screeners():
  ...
  # Download Nifty 50 benchmark series for Relative Strength comparison
  nifty_df = yf.download(
      "^NSEI", period="1y", interval="1d", progress=False
  )
  nifty_close = (
  ...
  for i, ticker in enumerate(tickers):
    progress_bar.progress((i + 1) / total_stocks)
    try:
      # Need 1 full year of history to calculate 52-week high accurately
      df = yf.download(ticker, period="1y", interval="1d", progress=False)
      if not df.empty and len(df) >= 200:
        if isinstance(df.columns, pd.MultiIndex):
          close = df[("Close", ticker)]
        ...
        # 1. 52-Week High Proximity Filter: Current price must be within 10% of 52-week high
        high_52w = high.max()
        pct_below_high = (high_52w - curr_close) / high_52w
        if pct_below_high > 0.10:  # More than 10% away from 52-week high
          continue
