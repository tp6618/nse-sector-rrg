...
data_sectors = fetch_data(timeframe, benchmark_ticker, sectors_dict)

if benchmark_ticker not in data_sectors.columns:
  # Fallback to default Nifty 50 if benchmark download fails
  benchmark_ticker = "^NSEI"
  data_sectors = fetch_data(timeframe, benchmark_ticker, sectors_dict)

if benchmark_ticker not in data_sectors.columns:
  st.error(
      "Benchmark data could not be retrieved. Please check your connection."
  )
else:
  ...
  if not ratio_df.empty:
    ...
    fig_sec.update_layout(
        ...
        xaxis=dict(range=[92, 114], dtick=2),
        yaxis=dict(range=[92, 108], dtick=2),
        height=680,
...
if not bull_flags:
  ...
  for idx, stock in enumerate(bull_flags):
    with cols_flag[idx % 3]:
      ...
      st.markdown("---")# PART 1: ALL SECTORS ROTATION CHART & SUMMARY
# ==========================================
...
if all_sectors_benchmark not in data_sectors.columns:
  ...
  if not ratio_df.empty:
    ...
    # Bottom-Right: Weakening (Soft Yellow/Orange)
    fig_sec.add_shape(
        type="rect",
        x0=100,
        y0=92,
    ...
    fig_sec.update_layout(
        ...
        height=680,
        template="plotly_white",
        showlegend=False,  # Labels are now directly on the chart like professional RRG terminals
    )
    st.plotly_chart(fig_sec, use_container_width=True)
...
if all_sectors_benchmark not in data_sectors.columns:
  ...
  if not ratio_df.empty:
    ...
    # Professional RRG Background Quadrant Fills (Matching Reference Image)
    fig_sec.add_shape(
        type="rect",
        x0=94,
        y0=100,
    ...
    for name in ratio_df.columns:
      ...
      fig_sec.add_trace(
          go.Scatter(
              ...
              name=name,
              text=[""] * (len(x_vals) - 1)
              + [name],  # Label only the latest head point
              ...
              line=dict(width=2),
              marker=dict(size=[6] * (len(x_vals) - 1) + [10], symbol="circle"),
          )
      )
    ...
    for name in ratio_df.columns:
      ...
      fig_sec.add_trace(
          go.Scatter(
              x=x_vals,
              y=y_vals,
              mode="lines+markers",  # Removed text clutter; using clean tooltips
              name=name,
              hovertemplate=(
                  f"<b>{name}</b><br>RS-Ratio: %{{x:.2f}}<br>RS-Momentum:"
                  " %{y:.2f}<extra></extra>"
